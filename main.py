...
def rayleigh(n):   return (np.random.randn(n) + 1j * np.random.randn(n)) / np.sqrt(2)
...
def af(snr):
    bits = np.random.randint(0, 2, NUM_BITS)
    tx   = mod(bits)
    h_sd, h_sr, h_rd = rayleigh(NUM_BITS), rayleigh(NUM_BITS), rayleigh(NUM_BITS)
